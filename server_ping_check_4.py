...
def detect_failure(p1):
    ...
    d_response = {}  # サーバの応答時間情報の格納場所を保管する辞書
    ll_server_ping = [] # サーバpingを格納するリスト
    ll_server_timelog = [] # サーバのタイムスタンプを格納するリスト
    server_cnt = 0
    ...
    for s_line in l_lines:
        ...
        if p1.flag == 'network':  # ネットワーク応答時間チェック
            ip_address, subnet_mask = server_address.split('/') #サブネットマスク計算
            network_address =  classify_network(ip_address, subnet_mask) # ネットワークアドレスでIPアドレスを置き換え、以下サーバー時と同処理
            server_address = str(network_address)
        ...
        if server_address not in d_response: # 新規サーバの登録
            ...
            ll_server_ping.append([server_cnt,ping_time]) # サーバのpingを記録
            ll_server_timelog.append([server_cnt,timestamp]) # サーバのタイムスタンプを記録
        ...
        if (len(ll_server_timelog[server_num])-1) == p1.average_cnt:  # 直近の所定回数の場合
            ave = sum(ll_server_ping[server_num])/len(ll_server_ping[server_num]) # 平均応答時間を計算
            if ave > p1.busy_ping:
                print(f"{p1.flag} {server_address} は過負荷状態です。(平均応答ping: {round(ave,1)})")
            ...
            if server_address not in d_failures:  # サーバが初めて故障した場合
                ...
                if server_address in d_failures:  # サーバが故障中である場合
                    ...
                    if int(d_failures[server_address]['count']) >= int(p1.count_limit):
                        ...
                        # 故障期間を出力するならここで出力
                        print(f"{p1.flag} {server_address} は {d_failures[server_address]['start_time']} から {timestamp} 間 故障しています。")
...
if __name__ == "__main__":
    if len(sys.argv) != 6:
        ...
        if 'network' == sys.argv[5].strip():
            p1.flag = sys.argv[5].strip()
        else:
            p1.flag = 'server'
